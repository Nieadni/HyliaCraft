...
public class BlueBokoblinEntity extends PathAwareEntity implements SmartBrainOwner<BlueBokoblinEntity>, InventoryOwner {
    private boolean hasHorn;
    private int hornTimer = 0;
    private SimpleInventory inventory = new SimpleInventory(9);

    protected BlueBokoblinEntity(EntityType<? extends PathAwareEntity> entityType, World world) {
    ...
    @Override
    public List<? extends ExtendedSensor<? extends BlueBokoblinEntity>> getSensors() {
        return List.of();
    }

    @Override
    public BrainActivityGroup<? extends BlueBokoblinEntity> getCoreTasks() {
        return SmartBrainOwner.super.getCoreTasks();
    }

    @Override
    public BrainActivityGroup<? extends BlueBokoblinEntity> getIdleTasks() {
        return BrainActivityGroup.idleTasks(
                new TargetOrRetaliateAndHorn<>()
                        .attackablePredicate(target -> target.isAlive() && !target.isInvulnerable()
                                && (target == BrainUtils.getTargetOfEntity((BlueBokoblinEntity) this) || BlueBokoblinEntity.isTargetableMob(target)))
                //Cook task
                //Eat task
        );
    }
    ...
    @Override
    public BrainActivityGroup<? extends BlueBokoblinEntity> getFightTasks() {
